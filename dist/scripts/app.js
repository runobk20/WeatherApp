...
class WeatherHelper {
    ...
    static createNextDayForecast(data) {
        ...
        nextDaysCard.className = 'd-flex justify-content-between p-3 next-days-card';

        const currentTemp = data.currentConditions.temp;

        currentTemp > 24 ? nextDaysCard.classList.add('bg-secondary-toned') : nextDaysCard.classList.add('bg-primary-toned');
            ...
            for(let i = 1; i < 6; i++) {
                ...
                cardImg.setAttribute('src', `dist/img/weather-icons/${data.days[i].icon}.svg`);
                currentTemp > 24 ? cardImg.className = 'little-card-img svg-secondary my-2' : cardImg.className = 'little-card-img svg-primary my-2'
    ...
    static render(data, type) {
        ...
        imgEl.setAttribute('src', `dist/img/weather-icons/${data.currentConditions.icon}.svg`);
